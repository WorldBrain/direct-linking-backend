...
var fetchResource = function () {
    var _ref = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee(url, type, progressKey, contentKey) {
        var response, data;
            ...
            while (1) {
                switch (_context.prev = _context.next) {
                    ...
                    case 3:
                        ...
                        _context.next = 6;
                        return response[type].bind(response)();
    ...
    return function fetchResource(_x, _x2, _x3, _x4) {
...
function updateBodyClasses() {
    ...
    var loading = !(state.fetchingHTML === 'done' && state.fetchMetadata === 'done' && state.fetchAnnotation === 'done');
    if (!loading) {
        body.classList.remove('loading');
    }

    if (state.metadata) {
        var embeddable = state.metadata.embeddable ? 'content-embeddable' : 'content-not-embeddable';
...
function deduceDocumentUrlWithoutProtocol() {
    ...
    var url = curPath.split('/').slice(2).join('/');
    var withoutTrailingSlash = url.substr(url.length - 1) === '/' ? url.slice(0, -1) : url;
...
function deduceAnnotationUrl() {
    var curPath = window.location.pathname;
    var id = curPath.split('/')[1];
    var url = '/' + id + '/annotation.json';
    return url;
}
...
Promise.all([fetchInnerHTML(), fetchMetadata(), fetchAnnotation(), injectGoogleFonts()]).then(function () {
    replaceInnerHTML();
...
},{"./_export":19,"./_hide":24,"./_iter-create":32,"./_iterators":36,"./_library":37,"./_object-gpo":43,"./_redefine":50,"./_set-to-string-tag":52,"./_wks":65}],34:[function(require,module,exports){
var ITERATOR = require('./_wks')('iterator');
